...
const FolderOrRepoReview = ({ orgFile }) => {
  const [folderOrRepoPath, setFolderOrRepoPath] = useState("");
  ...
  const handleSubmit = async (event) => {
    ...
    formData.append("folder_or_repo", folderOrRepoPath);
    formData.append("org_file", orgFile);

    try {
      const response = await axios.post("/api/review/folder-repo/", formData);
  ...
  return (
    <div className="acr">
      <div className={`review-container ${showSidebar ? "sidebar-pr-active" : ""}`}>
        <h1>Folder or Repository Review</h1>

        {/* Form Section */}
        <form onSubmit={handleSubmit} className="pr-form">
          <div className="form-container">
          <div className="textbox">
          <label>Folder Path or Repository URL:</label>
          <input
            type="text"
            value={folderOrRepoPath}
            onChange={(e) => setFolderOrRepoPath(e.target.value)}
            placeholder="Enter folder path or repo URL"
            required
          />
          </div>
...
export default FolderOrRepoReview;
